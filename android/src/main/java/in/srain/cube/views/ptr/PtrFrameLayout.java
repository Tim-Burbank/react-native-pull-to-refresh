...
public class PtrFrameLayout extends ViewGroup {
  ...
  protected void onFinishInflate() {
    ...
    if (childCount > 2) {
      ...
      errorView.setText("The content view in PtrFrameLayout is empty. Do you forget to specify its id in xml layout " +
        "file?");
      mContent = errorView;
      addView(mContent);
    }
  ...
  public boolean dispatchTouchEventSupper(MotionEvent e) {
    if (DEBUG) {
      PtrCLog.d(LOG_TAG, "dispatchTouchEventSupper : " + e.getAction());
    }
  ...
  public boolean dispatchTouchEvent(MotionEvent e) {
    ...
    switch (action) {
      ...
      case MotionEvent.ACTION_MOVE:
        ...
        //过滤
        if(!isMoved){
          offsetY = Math.abs(offsetY) < 2 ? 0:offsetY;
        }
        ...
        if (DEBUG) {
          PtrCLog.v(LOG_TAG, "ACTION_MOVE : canDoRefresh :%s ", canDoRefresh);
        }
  ...
  public boolean onTouchEvent(MotionEvent event) {
    PtrCLog.v(LOG_TAG, "onTouchEvent  action : " + event.getAction());
...
//          mScrollChecker.tryToScrollTo(mPtrIndicator.getOffsetToKeepHeaderWhileLoading(), mDurationToClose);
  ...
  private void sendDownEvent() {
    ...
    MotionEvent e = MotionEvent.obtain(last.getDownTime(), last.getEventTime(), MotionEvent.ACTION_DOWN, last.getX(),
